...
Before starting with our first analysis, some preliminary remarks.  
...
Missing values are indicated by `NA` or `NaN`. These are often found in data sets (where missingness is common) or as a result of invalid mathematical operations `log(-2)`. Be careful: **R** often returns values in place of errors, so it is good practice to check that the output is sensical. There is a special type for `NULL` pointers. Logicals are `TRUE` and `FALSE` in capital letters.
