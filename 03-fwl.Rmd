...
\end{align*}
have the same interpretation? For the matrix $\bX$ consisting of orthogonal regressors, this is 
clear. For the matrix $\bX'$, recall the FWL theorem: the regression coefficient $\beta_1'$ is the same as that of the regression 
$\Mmat_{\bx_2}\bs{y} = \Mmat_{\bx_2}\bf{1}_n + \bs{u}$ for $\bx_2 = (\bf{0}_{n_1}^\top, \bf{1}_{n_2}^\top)^\top$. The matrix 
$\Mmat_{\bx_2}$ is a block matrix, whose first $n_1 \times n_1$ block contains entries $n_1^{-1}$ and the rest of the entries is zero.
$\Mmat_{\bx_2}\bs{y}$ does not affect the last $n_2$ entries of $\bs{y}$, while $\Mmat_{\bx_2}\bf{1}_n = \bf{1}_n - \bx_2$. This 
