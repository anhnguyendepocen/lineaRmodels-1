...
### Oblique projections (exercise 1.4)
...
<!--chapter:end:02-computations.Rmd-->
...
The Frisch--Waugh--Lovell theorem is perhaps best explained using pictures. 
The following is taken from Davidson and McKinnon (1993) monograph 
We consider for simplicity the bivariate case and two non-parallel vectors $\mathbf{X}_1$ and $\mathbf{X}_2$. The yellow line illustrate the usual orthogonal decomposition $\bs{y} = \Hmat_{\mX}\by + \Mmat_{\mX}\by$, where $\Hmat_{\mX}\by$ is the segment $OB$ and $\Mmat_{\mX}\by$ is the segment $BA$. A similar decomposition can be performed in terms of the sole $\mX_1$, namely through $\Hmat_{\mX_1}\by$ corresponding to the line segment $OC$ and
$\Mmat_{\mX_1}\by$ corresponding to $CA$.

```{r fwl1, out.width = "85%", fig.cap = "Orthogonal decomposition into residuals and fitted values", echo = FALSE}
knitr::include_graphics("images/FWL1.png")
#knitr::include_graphics("images/FWL.pdf")
```

The right-angled triangle $ABC$ is orthogonal to $\mX_1$. We deduce that the segment $CB$ is $\Mmat_{\mX_1}\Hmat_{\mX}\by$. 

```{r fwl2, out.width = "85%", fig.cap = "Projection onto spane spanned by the regressors", echo = FALSE}
knitr::include_graphics("images/FWL3.png")
#knitr::include_graphics("images/FWL.pdf")
```

We may also look at the plane $\Sp(\mX)$ of the vector $\by$ and its fitted value. More specifically, consider the decomposition $\Hmat_{\mX}\by = \mX_1\hbb_1 + \mX_2\hbb_2$. This is represented by the parallelogram $ODBE$; the segments $OD$ and $EB$ correspond to the vector $\mX_2\hbb_2$, while $DB$ and $OE$ are shifted copies of $\mX_2\hbb_2$. 

```{r fwl3, out.width = "85%", fig.cap = "Triangle ABC", echo = FALSE}
...
```{r fwl4, out.width = "85%", fig.cap = "Triangle BCE", echo = FALSE}
knitr::include_graphics("images/FWL4.png")
#knitr::include_graphics("images/FWL.pdf")
```

We can deduce from the latter the formula for least squares. Looking at the residuals in the second picture, we see that the segment $AC$ is also the residual from the FWL regression. 

## Examples 

We have seen last week the centered $R^2_c$; the latter is equivalent to the coefficient of determination from the FWL regression 
