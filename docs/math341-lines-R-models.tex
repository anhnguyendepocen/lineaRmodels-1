...
\chapter{Projection matrices}\label{projection-matrices}}
...
to} \textbf{R}}. You may wish to look up the following chapters of the
...
Some \textbf{R} commands are fairly intuitive, especially if you want to
use your console as a calculator. Elementary functions such as
...
(\texttt{TRUE} and \texttt{FALSE}, note the capital letters). You can
assign \texttt{NULL} pointers to arguments. Special types exists for
missing values, namely \texttt{NA} or \texttt{NaN}. These can also be
obtained a result of invalid mathematical operations such as
\texttt{log(-2)}.
...
the \texttt{package} as an argument (must be a string).
...
\hypertarget{projection-matrices-1}{%
\subsection{Projection matrices}\label{projection-matrices-1}}
