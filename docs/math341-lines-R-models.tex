...
Suppose that
\(\dim(\mathcal{M}(\mathbf{X})) \neq \dim(\mathcal{M}(\mathbf{W}))\). An
oblique projection matrix is of the form
...
is such that \(\mathrm{im}(\mathbf{P})=\mathcal{M}(\mathbf{X})\), but
\(\mathrm{im}(\mathbf{I}-\mathbf{P})=\mathcal{M}(\mathbf{W}^\perp)\).
...
\CommentTok{#Create usual hat projection matrix and form P}
...
\CommentTok{#solve: matrix inverse}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Project a vector `vec' onto P, P transpose, I-P, I-(P transpose)}
...
\includegraphics{math341-lines-R-models_files/figure-latex/oblique_projection-1.pdf}
