...
\hypertarget{week-1-exercises}{%
\section{Week 1 exercises}\label{week-1-exercises}}

\hypertarget{oblique-projections}{%
...
\NormalTok{vec <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.9}\NormalTok{, }\FloatTok{-1.5}\NormalTok{)}
...
illustrate their use on the Motor Trend Car Road Tests dataset
(\texttt{mtcars}).
...
For more details about the complexity and algorithms underlying the
different methods, the reader is referred to these notes of
\href{Lee}{www.math.uchicago.edu/\textasciitilde{}may/REU2012/REUPapers/Lee.pdf}.
This material is \emph{optional}.
...
\NormalTok{X <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\DecValTok{1}\NormalTok{, }\KeywordTok{as.matrix}\NormalTok{(mtcars[,}\DecValTok{2}\OperatorTok{:}\KeywordTok{ncol}\NormalTok{(mtcars)]))}
...
We are now ready to fit a linear model with an intercept and a linear
effect for the weight. The model will be of the form
...
\NormalTok{X <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\DecValTok{1}\NormalTok{, wt)}
...
\NormalTok{fitted <-}\StringTok{ }\NormalTok{Hmat }\OperatorTok{%*%}\StringTok{ }\NormalTok{mpg}
\CommentTok{#Variance estimate and standard errors}
...
\NormalTok{std_err <-}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\KeywordTok{diag}\NormalTok{(s2 }\OperatorTok{*}\StringTok{ }\NormalTok{XtXinv))}
\end{Highlighting}
\end{Shaded}

The residuals \(\boldsymbol{e} = \boldsymbol{y} -\hat{\boldsymbol{y}}\)
can be interpreted as the \emph{vertical} distance between the
regression slope and the observation. This is illustrated in the
following graph. For each observation \(y_i\), a vertical line at
distance \(e_i\) is drawn from the prediction \(\hat{y}_i\).

Side remark: graphs and table should \emph{always} be properly labelled
(including units). The last line of the call to \texttt{plot} contains
cosmetic options that alter the display of the scatterplot --- you can
check for yourself the effects of removing any (all) of these additional
commands.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(mpg }\OperatorTok{~}\StringTok{ }\NormalTok{wt, }\DataTypeTok{xlab =} \StringTok{"weight (1000 lbs)"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"Fuel consumption (miles/US gallon)"}\NormalTok{, }
     \DataTypeTok{main =} \StringTok{"Fuel consumption of automobiles, 1974 Motor Trend"}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ mtcars,}
     \DataTypeTok{bty =} \StringTok{"l"}\NormalTok{, }\DataTypeTok{pch =} \DecValTok{20}\NormalTok{, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{35}\NormalTok{), }\DataTypeTok{xlim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{6}\NormalTok{)) }\CommentTok{#options to tweak the display}
...
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{nrow}\NormalTok{(X))\{}
  \KeywordTok{segments}\NormalTok{(}\DataTypeTok{x0 =}\NormalTok{ wt[i], }\DataTypeTok{y0 =}\NormalTok{ fitted[i], }\DataTypeTok{y1 =}\NormalTok{ fitted[i] }\OperatorTok{+}\StringTok{ }\NormalTok{res[i], }\DataTypeTok{col =} \DecValTok{2}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{vlines <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =}\NormalTok{ mtcars}\OperatorTok{$}\NormalTok{wt, }\DataTypeTok{y1 =}\NormalTok{ fitted, }\DataTypeTok{y2 =}\NormalTok{ fitted }\OperatorTok{+}\StringTok{ }\NormalTok{res)}
\NormalTok{ggg <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(mtcars, }\KeywordTok{aes}\NormalTok{(wt, mpg)) }\OperatorTok{+}\StringTok{ }
\StringTok{        }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}\StringTok{ }
\StringTok{        }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"weight (1000 lbs)"}\NormalTok{, }
            \DataTypeTok{y =} \StringTok{"Fuel consumption (miles/US gallon)"}\NormalTok{, }
            \DataTypeTok{title =} \StringTok{"Fuel consumption of automobiles, 1974 Motor Trend"}\NormalTok{) }\OperatorTok{+}
