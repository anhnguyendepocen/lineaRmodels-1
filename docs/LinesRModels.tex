...
package \texttt{ElemStatLearn}. - Briefly describe the dataset. - Look
at summaries of \texttt{lbph}. What likely value was imputed in places
of zeros in
\texttt{lbph\}\ (before\ taking\ the\ logarithm)?\ -\ Produce\ a\ plot\ of\ the\ pair\ of\ variables}lcavol\texttt{and}lpsa\texttt{on\ the\ log\ and\ on\ the\ original\ scale.\ Comment\ on\ the\ relationship\ between}lcavol\texttt{and}lpsa\texttt{.\ -\ \ Fit\ a\ linear\ model\ using\ the\ log\ cancer\ volume\ as\ response\ variable,\ including\ a\ constant\ and\ the\ log\ prostate\ specific\ antigen\ as\ covariates.\ Obtain\ numerically\ the\ OLS\ estimates\ \$\textbackslash{}hbb\$\ of\ the\ parameters,\ the\ fitted\ values\ \$\textbackslash{}hat\{\textbackslash{}bs\{y\}\}\$\ and\ the\ residuals\ \$\textbackslash{}bs\{e\}\$\ using\ the\ formulas\ given\ in\ class.\ \ -\ Compare\ the\ quantities\ you\ obtained\ with\ the\ output\ of\ the\ function}lm\texttt{.\ -\ Add\ the\ fitted\ regression\ line\ to\ the\ scatterplot\ of}lcavol\texttt{against}lpsa\texttt{.\ -\ \ Interpret\ the\ changes\ in\ cancer\ volume\ (not\ the\ log\ cancer\ volume),\ including\ any\ units\ in\ your\ interpretations.\ -\ \ Obtain\ the\ orthogonal\ projection\ matrix\ \$\textbackslash{}bf\{H\}\_\textbackslash{}mX\$\ and\ the\ OLS\ coefficients\ \$\textbackslash{}hbb\$\ using\ a\ SVD\ decomposition\ of\ \$\textbackslash{}mX\$\ (}svd\texttt{).\ -\ \ Compute\ the\ \$R\^{}2\_c\$\ coefficient\ and\ compare\ with\ the\ one\ in}summary\texttt{output\ of\ the}lm\texttt{function.\ What\ can\ you\ say\ about\ the\ explanatory\ power\ of\ the\ covariate}lpsa`?
