...
The centered \(R^2_c\) is equivalent to the coefficient of determination
...
exactly zero. Using \(R^2_c\) thus makes the comparison between the
adjusted linear model and the null model with only a constant, which
predicts each \(y_i (i=1, \ldots, n)\) by the average \(\bar{y}\).
