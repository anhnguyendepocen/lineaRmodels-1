...
#Diagonal elements (aka leverage points)
leverage <- diag(Hmat)
...
There are many types of residuals, which we cover in term. The model residuals are simply $\bs{e}=\Mmat_{\mX}\bs{y}$, which can be obtained through `resid` for `lm` objects.
...
In the sequel, we will look at calculation of various variants of the residuals. The first is the standardized residuals, also internally studentized residuals. These are obtained by scaling each $\bs{e}/s$ by the invidual variance terms $1-h_{ii}$ to make them homoscedastic.
...
You can safely skip this optional section, which contains advanced material that is not examinable.
...
We will replace the unknown parameters (here $\boldsymbol{\beta}$ and $\sigma^2$) by their best linear unbiased estimate. For $b=1, \ldots,  B$ where $B/\alpha \in \mathbb{N}$, repeat the following steps:
 \begin{enumerate}
\item sample $\boldsymbol{\varepsilon}_{b} \sim \mathcal{N}_n(\boldsymbol{0}_n, s^2\bf{I}_n)$ and form $\boldsymbol{y}_b = \mathbf{X} \hat{\boldsymbol{\beta}} + \boldsymbol{\varepsilon}_{b}$. 
...
#matplot(x = qt((1:n)/(n+1), df = n - p - 1), y = t(env$point), col = 2, add = TRUE, type = "l", lty = 1)
...
This section is also **optional material**.
...
The multiple $\mathrm{R}^2_c$ goes up the roof, but makes no sense here because it compares two models that are not nested (the model with a single mean versus which has no constant). A consequence of the removal of the intercept is that the average of the residuals is not zero anymore.

We now produce the quantile-quantile plots using.

```{r confints}
Q <- t(qr.Q(qr(Xmat1), complete = TRUE))
resQ1 <- (t(Q) %*% resid(lm_wind1))[-(1:2)]
#Function from Tutorial 3
confint.qqplot.ptw <- function(n, dist = "norm", ...){
  t(sapply(1:n, function(i){
...
lines(rankit, confint_boot[1,], col = "red", lty = 2)
lines(rankit, confint_boot[2,], col = "red", lty = 2)
#Theoretical bands based on order statistics distribution
lines(rankit, confint_ptwise[,1], col = "gray", lty = 3)
lines(rankit, confint_ptwise[,2], col = "gray", lty = 3)
```
...
labour <- read.table(url4, header = TRUE)
attach(labour)
...
## Legend
legend(x = "topleft", legend = c("Man","Woman"), col = c(2, 1), pch = 20, bty = "n")
