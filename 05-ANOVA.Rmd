...
and we reject the null hypothesis $\mu_1 = \cdots = \mu_L$ against the alternative that at least one group mean is different. 
...
# Could use weighted least squares
...
#Forward model selection - ASSUMES weight~gender is CORRECT...
anova(lm(weight.loss ~ gender * diet.type, data = diet))["gender", c("F value","Pr(>F)")] #gender not significant
anova(lm(weight.loss ~ diet.type * gender, data = diet))["diet.type", c("F value","Pr(>F)")] #diet type significant

#Alternatively
add1(M1, scope = c("diet.type","gender"), test = "F")
#Note that RSS is that of the more complext model, but not the full model
